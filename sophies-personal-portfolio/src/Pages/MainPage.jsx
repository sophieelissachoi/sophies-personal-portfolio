	...
	const experiences = [
		{
			...
			description: [
				"Co-authored a bidirectional real-time C#/.NET application (WPF, MVVM) that parses live radar telemetry from 9 tracked objects into human-readable format and transmits positioning commands via UDP to a Weibel radar system in real-time, giving operators direct command over radar positioning",
				"Engineered a multicast networking integration that streamed target data every 100 ms to the range's Common Display System, giving operators synchronized visibility into real-time tracking during military testing operations",
				"Presented system architecture, UML, and XAML GUI diagrams based on software requirements and OOP design at critical design reviews, revising based on stakeholder feedback before furthering development",
				"Authored a comprehensive test suite by writing unit tests in C#, debugging errors, and producing application test procedure documentation to complete SDLC validation and ensure seamless integration of new features",
			],
			type: "Internship",
		...
		},
		{
			...
			description: [
				"Ensured 24/7 uptime and security for 100+ club members by maintaining a production network built with a Proxmox hypervisor cluster (Linux), Docker container load balancer, NAS, SSO (Authentik), and VPNs",
				"Optimized faster VM read-write speed by configuring a dedicated storage switch connecting Proxmox hypervisor nodes to a NAS using CLI-based network configuration, improving storage bandwidth to 40 GB/s",
				"Architected an Agile-run sprint project that gives members visibility into a /18 network's subnets and deletes unused VMs to free up infrastructure, built with React, Go, RESTful APIs, PostgreSQL, OIDC SSO, and Proxmox for the website, and Docker, Kubernetes, and GitLab CI/CD for reliability and deployment",
				"Led Figma UI/UX design for a VM provisioning tool to abstract VM creation, cloning, and deletion for 20+ internal club members with React, TypeScript, Go, RESTful APIs, and Docker",
			],
			type: "Club",
			projects: ["EasyVM"],
			skills: [
		...
		},
		{
			...
			description: [
				"Programmed a full-stack application using React, JavaScript, PostgreSQL, and CSS, enabling scalable data management and real-time program oversight for a global NPO operating in 5 continents",
				"Deployed RESTful APIs in JavaScript with full CRUD functionality and Promise-based concurrent data fetching, minimizing user query time to <1s for a PostgreSQL database",
				"Collaborated in Agile sprints with pair programming and Git to ship React features against 2-week deadlines",
			],
			type: "Club",
		...
		},
		{
			...
			description: [
				"Engineered an AI-powered exploit generation engine using Python, RESTful APIs, and Anvil local chain, proving the validity of vulnerability reports by injecting proof-of-concept attacks on live contract state",
